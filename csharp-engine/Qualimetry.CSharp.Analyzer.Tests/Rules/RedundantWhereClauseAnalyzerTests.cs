...
public class RedundantWhereClauseAnalyzerTests
{
    ...
    [Fact]
    public Task UserDefinedWhereAndAny_IsClean()
    {
        const string source = """
            using System;
            ...
            {
                public Filterable Where(Func<int, bool> predicate) => this;
            ...
            public class Query
            {
                public bool Check(Filterable items)
                {
                    return items.Where(n => n % 2 == 0).Any();
                }
            }
            """;

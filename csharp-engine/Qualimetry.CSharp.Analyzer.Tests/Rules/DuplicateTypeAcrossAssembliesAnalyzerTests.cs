...
public class DuplicateTypeAcrossAssembliesAnalyzerTests
{
    ...
    [Fact]
    public async Task SameSimpleNameInDifferentNamespaces_IsClean()
    {
        const string libOneSource = """
        ...
        const string libTwoSource = """
            namespace Vitality.Test.AllureReport
            {
                public sealed class Attachment
                {
        ...
        MetadataReference libOne = MultiAssemblyTestHelper.CompileLibrary("AllureCommons", "1.0.0.0", libOneSource);
        MetadataReference libTwo = MultiAssemblyTestHelper.CompileLibrary("VitalityAllure", "2.0.0.0", libTwoSource);

...
public class PropertyGetterMutatesStateAnalyzerTests
{
    ...
    [Fact]
    public Task NullCoalescingLazyInit_IsClean()
    {
        const string source = """
            public class Lazy
            {
                ...
                public string Value => _cache ??= Compute();

                private static string Compute() => "value";
            }
            """;
    ...
    [Fact]
    public Task CompoundAssignmentInGetter_IsReported()
    {
        const string source = """
            public class Sequence
            {

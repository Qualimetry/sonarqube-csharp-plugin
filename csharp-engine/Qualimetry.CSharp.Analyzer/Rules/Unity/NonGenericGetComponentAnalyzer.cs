...
public sealed class NonGenericGetComponentAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void Analyze(SyntaxNodeAnalysisContext context)
    {
        ...
        if (context.SemanticModel.GetSymbolInfo(invocation).Symbol is not IMethodSymbol target
            || !IsUnityComponentLookup(target.ContainingType))
        {
            return;
        }
    ...
    private static bool IsUnityComponentLookup(INamedTypeSymbol? declaringType)
    {
        return UnityTypeNames.IsUnityType(declaringType, "Component")
            || UnityTypeNames.IsUnityType(declaringType, "GameObject")
            || UnityTypeNames.IsUnityType(declaringType, "MonoBehaviour");
    }
}

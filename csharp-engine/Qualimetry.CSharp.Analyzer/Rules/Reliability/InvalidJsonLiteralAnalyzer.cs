...
public sealed class InvalidJsonLiteralAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void Analyze(SyntaxNodeAnalysisContext context)
    {
        ...
        var targetsJson = (method == "Parse" && receiver is not null && ParseReceivers.Contains(receiver))
        ...
        if (!JsonText.IsValid(literal.Token.ValueText))
        {
    ...
    private static class JsonText
    {
        public static bool IsValid(string text)
        {
            ...
            if (!ParseValue(text, ref position))
            {
                return false;
            }
        ...
        private static bool ParseValue(string text, ref int position)
        {
        ...
        private static bool ParseObject(string text, ref int position)
        {
            position++;
            SkipWhitespace(text, ref position);
        ...
        private static bool ParseArray(string text, ref int position)
        {
            position++;
            SkipWhitespace(text, ref position);
            if (position < text.Length && text[position] == ']')

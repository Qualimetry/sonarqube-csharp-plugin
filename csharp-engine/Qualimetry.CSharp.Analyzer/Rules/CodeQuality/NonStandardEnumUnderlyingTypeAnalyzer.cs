...
public sealed class NonStandardEnumUnderlyingTypeAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void Analyze(SyntaxNodeAnalysisContext context)
    {
        ...
        var enumSymbol = context.SemanticModel.GetDeclaredSymbol(declaration, context.CancellationToken);
        if (enumSymbol is not null
            && enumSymbol.GetAttributes().Any(a => a.AttributeClass?.ToDisplayString() == "System.FlagsAttribute"))
        {
            return;
        }

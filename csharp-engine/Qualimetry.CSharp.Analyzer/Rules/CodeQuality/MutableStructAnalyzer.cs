...
public sealed class MutableStructAnalyzer : DiagnosticAnalyzer
{
    ...
    private static bool HasMutableState(MemberDeclarationSyntax member)
    {
        switch (member)
        {
            case PropertyDeclarationSyntax property when property.AccessorList is not null:
                return property.AccessorList.Accessors.Any(accessor => accessor.IsKind(SyntaxKind.SetAccessorDeclaration));

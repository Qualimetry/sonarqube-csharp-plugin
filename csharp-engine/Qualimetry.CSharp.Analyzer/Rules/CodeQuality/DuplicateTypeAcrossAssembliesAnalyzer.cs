...
public sealed class DuplicateTypeAcrossAssembliesAnalyzer : DiagnosticAnalyzer
{
    ...
    {
        var assembliesByFullName = new Dictionary<string, HashSet<IAssemblySymbol>>(StringComparer.Ordinal);
        ...
        foreach (IAssemblySymbol assembly in GetAssemblies(context.Compilation))
        {
            ...
            CollectTypes(assembly.GlobalNamespace, assembly, assembliesByFullName);
        }
        ...
        foreach (KeyValuePair<string, HashSet<IAssemblySymbol>> entry in assembliesByFullName)
        {
    ...
    private static void CollectTypes(
        ...
        Dictionary<string, HashSet<IAssemblySymbol>> assembliesByFullName)
    {
        foreach (ISymbol member in namespaceSymbol.GetMembers())
        {
            if (member is INamespaceSymbol childNamespace)
            {
                CollectTypes(childNamespace, assembly, assembliesByFullName);
                continue;
            }
            ...
            string fullyQualifiedName = type.ContainingNamespace.ToDisplayString() + "." + type.MetadataName;
            if (ShouldIgnoreMetadataName(fullyQualifiedName))
            {
                continue;
            }

            if (!assembliesByFullName.TryGetValue(fullyQualifiedName, out HashSet<IAssemblySymbol>? assemblies))
            {
                assemblies = new HashSet<IAssemblySymbol>(SymbolEqualityComparer.Default);
                assembliesByFullName[fullyQualifiedName] = assemblies;
            }

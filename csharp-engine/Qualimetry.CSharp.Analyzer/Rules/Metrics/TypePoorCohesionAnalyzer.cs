...
public sealed class TypePoorCohesionAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void Analyze(SyntaxNodeAnalysisContext context)
    {
        ...
        var fieldSets = methods
            .Select(method => new HashSet<IFieldSymbol>(
                GetAccessedInstanceFields(method, fieldSet, context.SemanticModel),
    ...
    private static IEnumerable<IFieldSymbol> GetAccessedInstanceFields(
        ...
        SemanticModel semanticModel)
    {
        foreach (SyntaxReference reference in method.DeclaringSyntaxReferences)
        {
            if (reference.GetSyntax() is not MethodDeclarationSyntax syntax
                || syntax.SyntaxTree != semanticModel.SyntaxTree)
            {
                continue;
            }
            ...
            foreach (SyntaxNode node in syntax.DescendantNodes())
            {
                ...
                if (semanticModel.GetSymbolInfo(identifier).Symbol is not IFieldSymbol field || field.IsStatic)
                {
                    continue;
                }
                ...
                else if (field.OriginalDefinition is IFieldSymbol original && fieldSet.Contains(original))
                {
